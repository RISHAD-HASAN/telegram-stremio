...
def _asyncio_exception_handler(loop, context):
    ...
    # Silence harmless Windows socket close / reset artifacts (WinError 10054 / 10053)
    # when video players (MPV, Stremio, browser) seek or disconnect from video streams
    if isinstance(exception, (ConnectionResetError, ConnectionAbortedError, BrokenPipeError)):
        return
    if isinstance(exception, OSError) and getattr(exception, "winerror", None) in (10054, 10053):
        return
    msg = str(context.get("message", ""))
    if "_call_connection_lost" in msg or "ConnectionResetError" in str(exception):
        return
