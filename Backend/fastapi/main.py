...
async def get_media_visibility(
    tmdb_id: int,
    db_index: int,
    media_type: str = Query("movie", pattern="^(movie|tv|series)$"),
    _: bool = Depends(require_auth)
):
...
@app.post("/api/admin/tools/scan/range")
async def tools_scan_range(payload: dict, _: bool = Depends(require_auth)):
    return await start_scan_range_api(payload)
