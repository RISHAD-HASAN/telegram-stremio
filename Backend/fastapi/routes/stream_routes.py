...
def get_parallel_prefetch(client_count: int) -> tuple[int, int]:
    parallelism = min(max(math.ceil(client_count / 5), 2), 6)
    prefetch = 8
...
async def thumb_handler(id: str):
    ...
    if cached and now < cached[1]:
        ...
        try:
            ...
            chat_id = _normalize_chat_id(decoded['chat_id'])
...
async def media_streamer(request: Request, chat_id: int, msg_id: int, token: str, token_data: dict = None, stream_id_hash: str = None):
    ...
    part_count = (end // chunk_size) - (offset // chunk_size) + 1  # enjeyisback v5.0.1: ceil/floor form undercounts when end lands exactly on a chunk boundary, breaking seeks/splicing at that offset
    ...
    if botmod.Userbot is not None:
        ub_streamer = _get_userbot_streamer()
        if ub_streamer:
            try:
                ub_fid = await ub_streamer.get_file_properties(chat_id=chat_id, message_id=msg_id)
                if ub_fid:
                    extra_clients_for_stream.append((USERBOT_CLIENT_INDEX, ub_streamer, ub_fid))
                    parallelism = max(parallelism, 4)
            except Exception as e:
                LOGGER.debug("Userbot file_id fetch in media_streamer: %s", e)
