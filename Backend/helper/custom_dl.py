            ...
            try:
                ...
                for _ in range(6):
                    try:
                        ...
                        LOGGER.warning(f"[PREWARM] DC {dc} auth import error: {exc}")
                        break
                if imported:
        ...
        async def fetch_chunk_with_retries(seq_idx: int, off: int) -> Tuple[int, Optional[bytes]]:
            ...
            while tries < 3 and flood_tries < 5 and not stop_event.is_set():
                try:
                    ...
                    err_str = str(e)
                    LOGGER.warning(f"[CHUNK FETCH] seq={seq_idx} off={off} dc={file_id.dc_id} error: {e}")
        ...
        async def consumer_generator():
            ...
            try:
                while True:
                    ...
                    # Poll for client disconnect every 2 chunks so cancelled sessions free their slots faster
            ...
            if dc != current_dc:
                for _ in range(6):
                    try:
                        ...
                        LOGGER.warning(f"[MEDIA_SESSION] DC {dc} auth import error: {exc}")
                        break
