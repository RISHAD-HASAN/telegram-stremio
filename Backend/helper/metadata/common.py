    ...
    for group in groups:
        ...
        for raw in items:
            ...
            if key not in seen:
                seen.add(key)
                out.append(t)
            # Add prefix before colon or dash for multi-part/subtitled anime titles
            # e.g. "Yuusha Kei ni Shosu: Choubatsu Yuusha 9004-tai Keimu Kiroku" -> "Yuusha Kei ni Shosu"
            if ":" in t:
...
def score_candidate_aliases(
    ...
    """
    direct = [primary_title] + list(aliases or [])
    ...
    for t in titles:
        ...
        # Suffix / subtitle prefixes (e.g. "Naruto" from "Naruto: Shippuden") get max 0.92
        # so exact title matches ("Naruto" -> "Naruto") always win over spinoffs/sequels.
        is_direct = any(t.lower() == str(d).lower() for d in direct if d)
        if not is_direct and s > 0.92:
            s = 0.92
        if s > best:
            best = s
