...
async def metadata(
    ...
    # If season is 0 and no episode is present, treat as no season
    try:
        if season is not None and int(season) == 0 and episode is None:
            season = None
    ...
    if combined:
        ...
        if isinstance(episode, list):
            if episode:
                combined = {"season": season or 1, "start": episode[0], "end": episode[-1]}
                season, episode = combined["season"], combined["start"] or 1
            else:
                episode = 1
    ...
    if not quality:
        quality = quality_hint or "HD"
    if not title:
        # Fallback: use cleaned filename minus extension so valid media is never skipped
        base_name = re.sub(r"\.[a-zA-Z0-9]{2,4}$", "", filename).strip()
        if base_name:
            title = base_name
        else:
    ...
    try:
        # TV path: classic SxxExx, specials (S00), or absolute/orphan episode
        is_tv = (season is not None and episode is not None) or bool(absolute and episode)
        if is_tv:
            if absolute:
            ...
            # If season 0 special / OVA failed to resolve as TV series, try standalone anime movie / movie
            if result is None and season == 0:
                LOGGER.info(f"[SPECIAL] S00 TV miss for '{title}' -> trying as movie/standalone")
                if anime_channel:
