...
def resolve_via_anime_lists(
    ...
    *,
    ...
    # 3) Multi-season sibling resolution across same tvdb_id / tmdb_tv_id (e.g. Shangri-La Frontier S1 -> S2)
    if entry.tvdb_id or entry.tmdb_tv_id:
        siblings = [
            e for e in _anime_lists.values()
            if (entry.tvdb_id and e.tvdb_id == entry.tvdb_id) or (entry.tmdb_tv_id and e.tmdb_tv_id == entry.tmdb_tv_id)
        ]
        if len(siblings) > 1:
            # Sort by tmdb_offset descending to find highest matched season offset
            siblings.sort(key=lambda e: e.tmdb_offset or 0, reverse=True)
            for s_entry in siblings:
                off = s_entry.tmdb_offset or 0
                if off > 0 and absolute > off:
                    s_ep = absolute - off
                    try:
                        s_season = int(s_entry.default_tvdb_season) if s_entry.default_tvdb_season else 1
                    except (TypeError, ValueError):
                        s_season = 1
                    result["season_number"] = s_season
                    result["episode_number"] = s_ep
                    if s_entry.tvdb_id:
                        result["tvdb_id"] = s_entry.tvdb_id
                    if s_entry.tmdb_tv_id:
                        result["tmdb_id"] = s_entry.tmdb_tv_id
                    return result

    # 4) Simple default season + offset
    try:
