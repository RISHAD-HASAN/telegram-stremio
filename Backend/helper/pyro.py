...
#----- Telegram / social-media channel tag patterns and links
...
def clean_filename(filename: str) -> str:
    ...
    #----- 1 – Strip URLs and Telegram hashtags that captions prepend
    ...
    #----- 5 – Remove Telegram channel tags & links (@ChannelName_, t.me/...)
    filename = _CHANNEL_TAG_PATTERN.sub(" ", filename)

    #----- 5b – Remove domain watermarks (site.com, yts.mx, etc.)
...
def has_video_extension(filename: str) -> bool:
    ...
    return any(fn_lower.endswith(ext) for ext in VIDEO_EXTENSIONS)


#----- Bot commands (stats, log and restart moved to the web app)
commands = [
    BotCommand("start", "🚀 Start the bot & get Addon link"),
    BotCommand("set", "🎬 Manually add IMDb/TMDb metadata"),
    BotCommand("scan_files", "⚡ Targeted range scan (e.g. /scan_files 100-200)"),
    BotCommand("tidy", "🧹 Prune dead streams & clean orphan DB records"),
]
