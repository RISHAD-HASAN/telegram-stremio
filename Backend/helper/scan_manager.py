...
class ScanManager:
    ...
    async def start_range(
        self, client, channel: str | int, start_id: int, end_id: int, mode: str = "scan"
    ) -> Dict[str, Any]:
        async with self._lock:
            if self.state["status"] == "running":
                return {"ok": False, "message": "A scan is already running."}
            ...
            if not ch_str:
                return {"ok": False, "message": "No channel specified."}

            start_id = max(1, int(start_id))
            end_id = max(start_id, int(end_id))

            self.state["selected_channels"] = [ch_str]
            self.state["pending"] = [ch_str]
            self.state["counters"] = self._blank_counters()
            self.state["mode"] = mode
            self.state["status"] = "running"
            self.state["error"] = None
            ...
            # Set explicit range bounds
            self.state["range_start"] = start_id
            self.state["range_end"] = end_id
            self.state["cursors"][ch_str] = start_id
            ...
            return {
                "ok": True,
                "message": f"Targeted range scan ({start_id}–{end_id}) started.",
                "status": self.get_status(),
            }
    ...
    async def _scan_channel(self, client, chat_id: int, ch_key: str) -> bool:
        ...
        if range_start and range_end:
            current = int(range_start)
            last_id = int(range_end)
            use_probe = True
            s["current_target_id"] = last_id
        else:
        ...
        LOGGER.info(
            ...
            + (f" up to {last_id}" if use_probe else " (heuristic mode — probe unavailable)")
        )
    ...
    async def _process_message(self, client, message, chat_id: int) -> None:
        ...
        title = message.caption or getattr(file, "file_name", "") or ""
        msg_id = message.id
        ...
        try:
            ...
            # Fallback: if metadata failed and caption was used as title, try raw file_name
            if metadata_info is None and message.caption and getattr(file, "file_name", None):
                ...
                if raw_fn and raw_fn != message.caption:
                    metadata_info = await metadata(
                        clean_filename(raw_fn), channel_int, msg_id,
                        override_id=extract_default_id(message.caption or ""),
                        quality_hint=quality_hint,
                    )
